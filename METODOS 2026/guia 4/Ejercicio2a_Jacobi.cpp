...
int main(int argc, char *argv[]) {
	...
	double array[TAM][TAM], resultados[TAM], x[TAM], x_viejo[TAM];
	double factor = 0, mayor = 0, temp = 0, suma_cocientes = 0, error = 5;
	...
	while(error > pow(10, -11) && (iteraciones < 10000))
	{
		...
		error = pow(error, 0.5);
		if(isnan(error) || isinf(error)) { printf("Diverge (Jacobi no converge para esta matriz)\n"); break; }
	}
